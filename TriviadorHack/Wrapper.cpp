...
void Wrapper::writeQuestionToFile() {
    if (!(this->isAnswered)) {
        ...
        if (qaAnswer > qabuffAnswer && qaAnswer > maxSingleDigit) {
            readQuestion.seekg(-1 * static_cast<int>(sizeof(QuestionAnswer)), std::ios::cur);
...
void Wrapper::triviadorHack(bool& goOn) {
    ...
    while (goOn) {
        while (!(this->timeToTakeQuestion()) && !(this->waitToTakeAnswer()) && !(this->timeToTakeQuestionSecond()) && !(this->timeToTakeQuestionThird()) && goOn) {
            sleep_until(system_clock::now() + milliseconds(100));
        }
        if (!goOn) {
            break;
...
bool Wrapper::changeQuestion(char* question, char* answer) {
    ...
    if (foundQuestion) {
        ...
        if (foundQuestion) {
            ...
            readQuestionSecond.seekg(-1 * static_cast<int>(sizeof(QuestionAnswerSecond)), std::ios::cur);
